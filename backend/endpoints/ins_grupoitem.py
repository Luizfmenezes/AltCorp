...
async def consulta_usuario(request: Request):
    ...
    exigeaprovacao = data.get("exigeaprovacao")

    if not sistema or not usuario or not senha or not status or not descricao or exigeaprovacao is None:
        raise HTTPException(status_code=400, detail="Campos 'sistema', 'usuario', 'senha', 'status', 'descricao' e 'exigeaprovacao' são obrigatórios")
    ...
    try:
        ...
        if auth == 1:
            ...
            try:
                query = "SELECT GRUPOITEM FROM GRUPOITEM WHERE DESCRICAO = ?"
                cursor.execute(query, (descricao,))
                row = cursor.fetchone()
                if row is not None:
                    return {
                        "mensagem": f"Grupo de item já cadastrado com id {row[0]}",
                        "status": 0,
                        "id": row[0]
                    }
            except pyodbc.Error as e:
                raise HTTPException(status_code=500, detail=f"Erro ao consultar grupo de item no banco: {str(e)}")

            try:
                query = "INSERT INTO GRUPOITEM (STATUS,DESCRICAO,EXIGEAPROVACAO,USUARIOCRIACAO) OUTPUT inserted.GRUPOITEM VALUES (?,?,?,?)"
                valores = (status, descricao, exigeaprovacao, id_user)
                ...
                if linha is None:
                    ...
                    conn.commit()
                    return {
                        "mensagem": f"grupo registrado com sucesso no id {id_grupoitem}",
                        "status": 1,
                        "id": id_grupoitem
                    }
            except pyodbc.Error as e:
